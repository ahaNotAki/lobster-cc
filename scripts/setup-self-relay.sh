...
for cidr in "${CIDRS[@]}"; do
    ...
    if [ "$cidr" = "0.0.0.0/0" ]; then
        echo "  REFUSED: will not open the relay port to 0.0.0.0/0"
        exit 1
    fi
...
if [ "$DRY_RUN" = true ]; then
    echo "  + create lobster-relay user + dirs (/opt/lobster-relay, /var/lib/lobster-relay)"
    ...
    ssh -i "$SSH_KEY" "$HOST" "sudo rm -rf /opt/lobster-relay/remote_control; \
        ...
        sudo pip3 install --quiet pycryptodome aiohttp 2>/dev/null || true"
