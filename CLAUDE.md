...
- `core/router.py` — Slash command parsing (`/status`, `/cancel`, `/new`, `/clear`, `/memory`, etc.) vs task creation. Scheduling is handled by Claude Code's scheduler MCP plugin (natural language).
- `core/executor.py` — Task queue orchestration: picks queued tasks, runs them sequentially via AgentRunner, with streaming output. Injects memory context, WeCom MCP tool hints, and output format guidelines before each task. Captures thinking for dashboard. Sends queue confirmation when tasks are waiting.
- `core/runner.py` — Spawns `claude -p --output-format stream-json --verbose --include-partial-messages --resume <session_id>`. Parses thinking blocks, text deltas, model info from stream events.
