...
- `core/router.py` — Slash command parsing (`/status`, `/cancel`, `/new`, `/clear`, `/long <task>` for 6-hour long tasks, etc.) vs task creation. Supports custom slash commands from agent profile. Scheduling is handled by Claude Code's scheduler MCP plugin (natural language).
- `core/executor.py` — Task queue orchestration: picks queued tasks, runs them sequentially via AgentRunner, with streaming output. Injects WeCom MCP tool hints, profile-aware output style hints, output format guidelines, and **task history recall hints** (`recall_tasks` / `get_task_detail`) before each task. Supports per-task model override from profile. Captures thinking for dashboard. Sends queue confirmation when tasks are waiting. Saves task archive + Claude-generated summaries on completion.
...
- `dashboard/routes.py` — Read-only dashboard web UI with password auth, IP lockout. REST API endpoints: `/api/status` (full system state), `/api/task/{task_id}` (task details for expandable UI), `/api/tab/{agent_id}/{tab_id}` (custom tab data).
...
- `dashboard/tabs.py` — Custom tab system: loads `.dashboard-tabs.json` from working dir, serves tab data via template renderers (table, key-value, chart, HTML). Path security (realpath validation), 1MB size limit.
- `dashboard/static/dashboard.html` — Lobster aquarium WebUI with multi-agent support, expandable task details with emoji status tags, custom tabs grouped by agent name with table/chart/HTML renderers
...
**Agent Profile System**: Per-agent preferences stored in `.agent-profile.yaml` in the working directory, managed by `ProfileManager` with hot-reload (mtime check) and audit trail (timestamped snapshots in `.agent-profile-history/`). Profiles control output style (language, format, max length), model selection (per-task-type regex overrides), notification prefs, and custom slash commands. Claude can self-configure via MCP tools (`get_agent_config`, `set_agent_config`, `list_agent_config`, `reset_agent_config`) exposed by `mcp/profile_server.py`. The profile is automatically bootstrapped on first access, extracting initial prefs from existing `.system-prompt.md` if present. All profile fields are backward compatible — if profile_manager is None, components fall back to config.yaml defaults.

**Custom Dashboard Tabs**: Agents can create custom data views in the dashboard by placing `.dashboard-tabs.json` in their working directory. Supports three tab types: `data` (JSON → table or key-value layout), `chart` (JSON → line/bar charts via Canvas 2D), and `html` (iframe-sandboxed HTML). Tab definitions are loaded by `dashboard/tabs.py` with path security (realpath validation to prevent traversal). Tab data is served on-demand via `/api/tab/{agent_id}/{tab_id}`. The dashboard UI groups tabs by agent name with visual separators.
